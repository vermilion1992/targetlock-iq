...
**Tests:** `npm run test` (95 tests — drilling, steering, reports, assumptions, validation, test scenarios)
...
| **UI** | Simple = **Action plan** + KPIs + basic charts; Advanced = grouped tabs (Trajectory · Steering feasibility · QA/QC · Decisions · **Math reference** · **Validation** · Setup/assumptions); **Guide** (overview + 10-step tour) |
...
| **Test scenarios** | **7 built-in synthetic holes** (on plan, drift, recoverable, motor/Navi, wedge/branch, QA jump, invalid CSV) — load from **Scenario lab** modal; scenario name on exports. See [test-scenarios.md](./test-scenarios.md) |
...
| **Quality** | 87 unit tests; `tsc --noEmit`; production build |
