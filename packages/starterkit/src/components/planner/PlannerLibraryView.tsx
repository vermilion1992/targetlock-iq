...
export function PlannerLibraryView({
  ...
  const holes = useMemo(
    () => listPlannerHoles(library, { includeArchived: true }),
    [library]
  );
  ...
  const qaByHoleId = useMemo(() => {
    ...
    const allPrograms = derivePlannerPrograms(library);
  ...
  const rows = useMemo(() => {
    return holes
      ...
      .filter((row) => {
        ...
        if (
          programFilter !== "all" &&
          (row.programName ?? "Unassigned") !== programFilter
        ) {
          return false;
        }
  ...
  return (
    <div className="planner-library-view">
      <AdvancedTabHero
        eyebrow="Hole planner"
        title="Plans"
        copy="Dense program library — status, readiness, QA, and actions per row."
        actions={
          <>
            <button
              type="button"
      ...
      <PlannerSubPanel kicker="Library" title="Program holes">
